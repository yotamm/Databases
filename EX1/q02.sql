...
FROM inventory 
JOIN (SELECT inventory_id, rental_id, rental_date
	  FROM rental
	  WHERE	YEAR(rental_date) = 2005 AND MONTH(rental_date) = 7) 
AS july_rentals ON inventory.inventory_id = july_rentals.inventory_id
GROUP BY inventory.store_id
HAVING count(july_rentals.rental_id) >= ALL (SELECT count(rental_id)
											 FROM inventory AS i 
											 JOIN (SELECT inventory_id, rental_id, rental_date
												   FROM rental
												   WHERE YEAR(rental_date) = 2005 AND MONTH(rental_date) = 7) 
											 AS jr ON i.inventory_id = jr.inventory_id
											 GROUP BY i.store_id)
UNION
SELECT store_id
FROM inventory 
